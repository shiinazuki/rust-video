...
use crate::{ModuleLoader, ModulePath, ModuleSource, transpiler::TypeScript};
...
impl ModuleLoader for FsModuleLoader {
    ...
    fn load(&self, specifier: &str) -> Result<ModuleSource> {
        ...
        match path_extension {
            "ts" => TypeScript::compile(fname, &source).map_err(|e| anyhow::anyhow!(e.to_string())),
            _ => Ok(source),
        }
    }
}
...
impl ModuleLoader for UrlModuleLoader {
    ...
    fn load(&self, specifier: &str) -> Result<ModuleSource> {
        ...
        let source = match ureq::get(specifier).call()?.into_body().read_to_string() {
            Ok(source) => source,
