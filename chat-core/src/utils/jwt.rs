...
impl ChatEncodingKey {
    pub fn load(priv_pem: &str) -> Result<Self, pem::PemError> {
        let pem = pem::parse(priv_pem.as_bytes())?;
        let encoding_key = EncodingKey::from_ed_der(pem.contents());
        Ok(Self(encoding_key))
    }

    pub fn sign(&self, user: impl Into<User>) -> Result<String, jsonwebtoken::errors::Error> {
        let claims = Claims {
            sub: user.into(),
...
impl ChatDecodingKey {
    pub fn load(pub_pem: &str) -> Result<Self, pem::PemError> {
        let pem = pem::parse(pub_pem.as_bytes())?;
        let decoding_key = DecodingKey::from_ed_der(pem.contents());
        Ok(Self(decoding_key))
    }

    pub fn verify(&self, token: &str) -> Result<User, jsonwebtoken::errors::Error> {
