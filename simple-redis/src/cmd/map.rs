...
impl TryFrom<RespArray> for GetCommand {
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match args.next() {
            Some(RespFrame::BulkString(key)) => Ok(GetCommand {
...
impl TryFrom<RespArray> for SetCommand {
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match (args.next(), args.next()) {
            (Some(RespFrame::BulkString(key)), Some(value)) => Ok(SetCommand {
                key: String::from_utf8(key.0)?,
                value,
            }),
            _ => Err(CommandError::InvalidArgument("Invalid key".to_string())),
...
mod tests {
    ...
    fn test_get_from_resp_array() -> Result<()> {
        ...
        let result: GetCommand = frame.try_into()?;
    ...
    fn test_set_from_resp_array() -> Result<()> {
        ...
        let result: SetCommand = frame.try_into()?;
        assert_eq!(result.key, "hello");
