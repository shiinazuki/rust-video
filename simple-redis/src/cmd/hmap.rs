...
impl TryFrom<RespArray> for HGetAllCommand {
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match args.next() {
            Some(RespFrame::BulkString(key)) => Ok(HGetAllCommand {
                key: String::from_utf8(key.0)?,
            }),
...
mod tests {
    ...
    fn test_hget_from_resp_array() -> Result<()> {
        ...
        let result: HGetCommand = frame.try_into()?;
    ...
    fn test_hgetall_from_resp_array() -> Result<()> {
        ...
        let result: HGetAllCommand = frame.try_into()?;
        assert_eq!(result.key, "map");
    ...
    fn test_hset_from_resp_array() -> Result<()> {
        ...
        let result: HSetCommand = frame.try_into()?;
        assert_eq!(result.key, "map");
        assert_eq!(result.field, "hello");
