...
mod test {
    use super::*;
    use crate::{NullBulkString, RespNullArray};
    ...
    fn respv2_null_bulk_string_should_work() {
        ...
        let frame = RespFrame::decode(&mut buf).unwrap();
        assert_eq!(frame, RespFrame::NullBulkString(NullBulkString));
    }
    ...
    #[test]
    fn respv2_bool_length_should_work() {
        let buf = b"#t\r\n";
        let len = RespFrame::expect_length(buf).unwrap();
        assert_eq!(len, buf.len());
